...
### Setp 4 
- depth 카메라 기반으로 촬영하는 물체의 정확한 크기 측정
- 측정된 크기 기반으로 45cm 이하에서는 물체가 커진 만큼 커진 크기를 역으로 계산하여 물체와 카메라까지의 거리 계산.
- 물체가 공중에 떠 있을 경우, 거리 측정시 정확도 하락
	- 하락 이유 분석 : 바운딩 박스의 구조적 한계로 인한 배경 혼입
		- 현재 물체 인식시 ROI를 활용, Depth 카메라의 한계로 모서리 부분이 비어 있어 뒤에 있는 배경의 깊이 값 입력. 

# 실행
### teminal 1
```
source ~/ros2_venv/bin/activate
export PYTHONNOUSERSITE=1 # 가상환경 내의 numpy 버전 고정 (로컬 버전과 충돌 최소화)
...
<img width="3840" height="2160" alt="Image" src="https://github.com/user-attachments/assets/1048295b-22ef-4b08-a5b4-af6309d0dba9" />
